...
fn main() -> osm_api::Result<()> {
    ...
    let bounds = if env::args().count() == 6 {
        BoundaryRect {
            min_x: env::args()
                .nth(2)
                .expect("Min x must be provided")
                .parse()
                .expect("Min x must be a float"),
            min_y: env::args()
                .nth(3)
                .expect("Min y must be provided")
                .parse()
                .expect("Min y must be a float"),
            max_x: env::args()
                .nth(4)
                .expect("Max x must be provided")
                .parse()
                .expect("Max x must be a float"),
            max_y: env::args()
                .nth(5)
                .expect("Max y must be provided")
                .parse()
                .expect("Max y must be a float"),
        }
    } else {
    ...
    let wkb = manager
        .get_geometry_as_wkb(&obj, &bounds)
        .expect("Could not get WKB")
        .unwrap();
