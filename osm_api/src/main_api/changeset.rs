...
impl From<RawChangeset> for Changeset {
    fn from(raw: RawChangeset) -> Self {
        let bounds = if let (Some(min_x), Some(min_y), Some(max_x), Some(max_y)) = (raw.min_lon, raw.min_lat, raw.max_lon, raw.max_lat) {
        Some(BoundaryRect {
            min_x,
            min_y,
            max_x,
        ...
        let state = if raw.is_open { ChangesetState::Open } else { ChangesetState::Closed(raw.closed_at.unwrap()) };
