...
impl TryFrom<RawOSMChange> for OSMChange {
    ...
    fn try_from(value: RawOSMChange) -> Result<Self> {
        Ok(OSMChange {
            create: value.create.into_iter().map(|o| o.try_into()).collect::<Result<Vec<OSMObject>>>()?,
            modify: value.modify.into_iter().map(|o| o.try_into()).collect::<Result<Vec<OSMObject>>>()?,
            delete: value.delete.into_iter().map(|o| o.try_into()).collect::<Result<Vec<OSMObject>>>()?,
        })
    }
}