...
pub fn infer_additional_relationships_for_entity(mut entity: &mut Entity, db: &AreaDatabase, mut street_names_cache: &mut HashMap<String, String>) -> Result<Vec<EntityRelationship>> {
    trace!("Inferring address relationships for {} {}", entity.discriminator, entity.id);
...
pub fn infer_additional_relationships_for(db: &AreaDatabase) -> Result<Vec<EntityRelationship>> {
    ...
    let mut executor = EntitiesQueryExecutor::new(&query);
        ...
        for mut entity in results {
            let mut new_relationships = infer_additional_relationships_for_entity(&mut entity, &db, &mut street_names_cache)?;
