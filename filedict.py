...
Author: Erez Shinan
Date  : 31-May-2009
Modified to work under python 3. Note that if you want persistence between python restarts, only ints are safe as key, because the hashing is now randomized.
"""
...
class FileDict(collections.UserDict):
    ...
    def items(self):
        return ((k, self.__unpack(v)) for k, v in self.__conn.execute('select key,value from %s;'%self.__tablename))
