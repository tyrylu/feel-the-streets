...
use zstd_safe::{CCtx, CParameter, DCtx, CDict};
...
impl<'a> ZstdContext<'a> {
    pub fn new(compression_level: i32, dictionary: Option<&[u8]>) -> Self {
        ...
        let cdict = match dictionary {
            Some(ref dict_data) => {
                ...
                cctx.ref_cdict(&dict).expect("Failed to associate compression dictionary");
                Some(dict)
            },
            None => {
                cctx.set_parameter(CParameter::CompressionLevel(compression_level)).expect("Failed to se the compression level");
                None
            }
            };
        if let Some(ref dict_data) = dictionary {
                            dctx.load_dictionary(&dict_data).expect("Failed to associate decompression dictionary");
        }
    ...
    pub fn decompress(&mut self, compressed: &[u8]) -> Result<Vec<u8>> {
        ...
        let start = Instant::now();
        self.decompression_context.decompress(&mut original, &compressed)?;
