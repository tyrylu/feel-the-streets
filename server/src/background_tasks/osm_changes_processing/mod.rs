...
fn process_osm_change(sn: u32, r_client: &ReplicationApiClient, m_client: &MainAPIClient, manager: &OSMObjectManager, server_db: &Connection) -> Result<()> {
    ...
    for modified in &change.modify {
        if changeset_might_be_interesting(modified.changeset, &mut changeset_interests, m_client, server_db) {
    ...
    for modified in interesting_modified {
        handle_modification(modified, manager, server_db, &mut record, &mut changes_container)?;
    }
    let mut interesting_created = Vec::with_capacity(change.create.len());
    for created in &change.create {
        if changeset_might_be_interesting(created.changeset, &mut changeset_interests, m_client, server_db) {
    ...
    for created in interesting_created {
        handle_creation(created, manager, server_db, &mut record, &mut changes_container)?;
    }
    for deleted in &change.delete {
        if changeset_might_be_interesting(deleted.changeset, &mut changeset_interests, m_client, server_db) {
    ...
    for (area_id, info) in changes_container.iter_mut() {
        info!("Processing {} semantic changes for area {}.", info.changes.len(), area_id);
        ...
        if !stream.exists()? || !stream.should_publish_changes()? {
            info!("Not publishing the changes of area {}, no clients or forced redownload.", area_id);
        } else {
            ...
            debug!("Publishing {} changes for area {}...", info.changes.len(), area.osm_id);
...
fn handle_modification<'a>(object: &'a OSMObject, manager: &OSMObjectManager, conn: &Connection, record: &mut TranslationRecord, changes: &mut SemanticChangesContainer<'a>) -> Result<()> {
    let start = Instant::now();
    let areas = Area::all_containing(conn, &manager.get_geometry_as_wkb(object, &BoundaryRect::whole_world())?.unwrap())?;
    trace!("All db areas containing the entity retrieved in {:? }, they are: {:?}", start.elapsed(), areas);
    ...
    for area in areas {
        ...
        if let Some((new_entity, new_related_ids)) = translator::translate(object, &bounds, manager, record)? {
            ...
            if db_containing_area_ids.contains(&area.osm_id) {
                debug!("Object {} was modified in area {}.", new_entity.id, area.osm_id);
                ...
                if object.unique_id().as_str() == osm_api::area_id_to_osm_id(area.osm_id) {
                    if let SemanticChange::Update { ref property_changes, .. } = semantic_change {
                        for change in property_changes {
                            match change {
                                EntryChange::Update { ref key, new_value, .. } if key == "geometry" => {
                                    changes.update_geometry_for(area.osm_id, BASE64_STANDARD.decode(new_value.as_str().unwrap()).expect("Could not decode"));
                                }
                                _ => { }
                            }
                        }
                ...
                debug!("Due to a modification of object {}, it now should be also in area {}", new_entity.id, area.osm_id);
    ...
    for area_id in db_containing_area_ids {
        if !geometric_containing_area_ids.contains(&area_id) {
                        debug!("Object {} is no longer part of area {}.", object_id, area_id);
...
fn handle_geometry_change(entity_id: &str, conn: &Connection, manager: &OSMObjectManager, changes: &mut SemanticChangesContainer) -> Result<()> {
    let object = manager.get_object(entity_id)?.expect("Modified object managed to disappear");
    for area_id in db::areas_containing(entity_id, conn)? {
        let old_geom = AreaDatabase::open_existing(area_id, true)?.get_entity(entity_id)?.expect("The summary says we have the entity, but it is not there").geometry;
        let bounds = Area::bounds_of(area_id, conn)?;
        let new_geom = manager.get_geometry_as_wkb(&object, &bounds)?.expect("Could not create new geometry");
        let geom_change = EntryChange::updating("geometry", BASE64_STANDARD.encode(&old_geom).into(), BASE64_STANDARD.encode(&new_geom).into());
        changes.add_change(area_id, SemanticChange::updating(entity_id, vec![geom_change], vec![], vec![]));
...
fn handle_creation<'a>(object: &'a OSMObject, manager: &OSMObjectManager, conn: &Connection, record: &mut TranslationRecord, changes: &mut SemanticChangesContainer<'a>) -> Result<()> {
...
fn handle_deletion(object: &OSMObject, conn: &Connection, manager: &OSMObjectManager, changes: &mut SemanticChangesContainer) -> Result<()> {
    let object_id = object.unique_id();
...
fn to_create_change(entity: Entity, ids: impl Iterator<Item=String>) -> SemanticChange {
    SemanticChange::creating(entity.id.to_string(), entity.geometry, entity.discriminator.to_string(), entity.data, entity.effective_width, child_ids_to_rels(ids))
}

fn to_update_change(new_entity: Entity, new_related_ids: impl Iterator<Item=String>, area_id: i64) -> Result<SemanticChange> {
    let area_db = AreaDatabase::open_existing(area_id, true)?;
    let old_entity = area_db.get_entity(&new_entity.id)?.expect("A modified entity is not in the db");
    ...
    let rel_changes = raw_rel_changes.into_iter().map(|c|
        match c {
    ...
    Ok(SemanticChange::updating(&old_entity.id, property_changes, data_changes, rel_changes))
}

    fn child_ids_to_rels(ids: impl Iterator<Item=String>) -> Vec<RootedEntityRelationship> {
        ids.map(|id| RootedEntityRelationship::new(&id, EntityRelationshipKind::OSMChild)).collect()
    }
